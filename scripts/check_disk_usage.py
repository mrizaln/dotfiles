...
class Partition:
    ...
    def __init__(self, name: str = "", path: str = "", size: int = 0, usedSize: int = 0) -> None:
        self.name: str = name if len(name) < self.NAME_MAX_LEN else name[:self.NAME_MAX_LEN-3] + "..."
        self.path: str = path if len(path) < self.PATH_MAX_LEN else path[:self.PATH_MAX_LEN-3] + "..."
        self.size: int = size
    ...
    def print(self, terminalWidth : int):
        ...
        info = f"{formatBytes(free):>6} free"
        ...
        if (width - whole_width - 1) < 0:
          part_char = ""
        bar = "[" + "█" * whole_width + part_char + " " * (width - whole_width - 7) + "]" if terminalWidth >= 100 else ''

        line = f"{self.name:<{self.NAME_MAX_LEN}} {self.path:<{self.PATH_MAX_LEN}} {info}  {bar} {round(progress*100, 1):<4} %"
        print(line)
